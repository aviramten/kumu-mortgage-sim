/**
 * ComparisonBalanceChart — line chart showing outstanding principal balance
 * over time for up to 3 mixes. Sibling to ComparisonChart (monthly payment),
...
import type { ComparisonEntry } from './ComparisonChart'

// ---------------------------------------------------------------------------
// Y-axis tick formatter — ₪1.2M / ₪500K
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Helpers — sum closingBalance across all tracks per month, per mix entry
// ---------------------------------------------------------------------------
function buildBalanceData(entries: ComparisonEntry[]) {
  const maps: Map<number, number>[] = entries.map(() => new Map())

  entries.forEach(({ result }, i) => {
    for (const tr of result.trackResults) {
      for (const row of tr.rows) {
        maps[i].set(row.month, (maps[i].get(row.month) ?? 0) + row.closingBalance)
      }
    }
  })
  ...
  return Array.from(allMonths)
    ...
    .map((month) => {
      ...
      entries.forEach((_, i) => {
        point[`mix${i}`] = Math.max(0, maps[i].get(month) ?? 0)
      })
      return point
    })
}
...
// Public component
// ---------------------------------------------------------------------------
interface ComparisonBalanceChartProps {
  entries: ComparisonEntry[]
}

export function ComparisonBalanceChart({ entries }: ComparisonBalanceChartProps) {
  const { theme } = useThemeStore()
  const isDark    = theme === 'dark'

  const chartData = useMemo(() => buildBalanceData(entries), [entries])
